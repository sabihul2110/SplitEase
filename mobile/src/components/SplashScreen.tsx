...
// /*
//  * ── SIZE ─────────────────────────────────────────────────────
//  *  Compact like ChatGPT / Telegram: ~120dp circle.
//  */
...
// /*
//  * ── EXACT LOGO PATHS ─────────────────────────────────────────
//  *  Taken directly from favicon-solid.svg, re-centred to
//  *  viewBox="0 0 512 512" with translate(256,256) scale(1.8)
...
// /* ── Slice — fades in with scale punch ───────────────────────*/
// function Slice({
//   d, fill, delay,
// }: { d: string; fill: string; delay: number }) {
...
//     // RN SVG doesn't support transform on Path via animatedProps,
//     // so opacity-only is the safe cross-platform choice.
//   }));
...
//   /*
//    * ONE animated wrapper — circle bg + exact logo SVG.
//    * scale: 0.01 → spring to 1  (Telegram pop-in)
//    * rotate: single entry spin only, then STOPS.
//    *   (No idle spin — the logo has an X cross, slow spin
//    *    looks off. One clean spin on entry like Telegram.)
//    */
...
//     // Single entry spin: 0 → 360° in 700 ms, then hold.
...
//     // Fade to black
//     opacity.value = withDelay(T.fadeOut,
...
//       {/* ── LOGO ── */}
//       <AnimatedView style={[styles.logoCircle, logoStyle]} pointerEvents="none">
//         <Svg
//           width="100%"
//           height="100%"
//           viewBox="-216 -216 432 432"   // centres at 0,0; fits scale(1.8)×120
//         >
//           {/* Background disc — fills the circle clip */}
//           <Circle cx="0" cy="0" r="216" fill="#13141c" />

//           {/* 4 slices — exact paths from favicon-solid.svg */}
//           <G scale={1.8}>
//             {SLICES.map((s, i) => (
...
//             {/* X cross — the defining detail of your logo */}
//             <Line
...
//     // Subtle blue glow (iOS only; harmless on Android)
//     shadowColor:    '#3b82f6',
...
const T = {
  ...
  textIn:  750,
  dotsIn: 1050,
  fadeOut: 2300,   // logo + text + dots all start fading here
  totalMs: 2700,
};
...
export default function SplashScreen({ onFinish }: Props) {
  ...
  // Single shared opacity — drives logo, text AND dots together
  ...
  const logoStyle = useAnimatedStyle(() => ({
    ...
    opacity: masterOpacity.value,   // ← uses master
  }));

  // Text opacity = min of its own reveal anim × master fade
  // Simplest cross-platform approach: wrap all three in one master container
  ...
  // Squircle corner radius — ~22% of size matches the touch icon
  ...
  return (
    <View style={styles.container}>
      ...
      {/* ── LOGO (squircle) ── */}
      <AnimatedView
        style={[styles.logoSquircle, { borderRadius: squircleRadius }, logoStyle]}
        pointerEvents="none"
      >
        <Svg
          width="100%"
        ...
        >
          {/* Background fill — square with rounded clip handled by View */}
          <Rect x="-216" y="-216" width="432" height="432" fill="#13141c" />
